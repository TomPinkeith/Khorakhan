...
<?
  ...
  $query = "SELECT data, autore, messaggio, idaut, luogo FROM chat WHERE luogo=\"$stanza\" OR luogo=\"ALL\" OR luogo=\"$stanza".":"."$nome\" ORDER BY data DESC";
  $passo1 = mysql_query($query);

  $i = 1;

    while (list($data, $autore, $mess, $idaut, $luogo) = mysql_fetch_array($passo1)) {
      ...
      if ($oggi <= $data) {
        ...
        if ($luogo == "$stanza".":"."$nome") {
          ...
          if ($perm == 2 OR $perm == 3 OR $perm == 4 OR $perm == 5) {
